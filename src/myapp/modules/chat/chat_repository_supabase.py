...
class ChatRepositorySupaBase(IChatRepo):
    ...
    def add(self, user_id: int, frnd_uid: int):
        ...
        with self.db.transaction() as supabase:
            ...
            supabase.from_("user_chat").insert(
                {"userid": user_id, "chatid": chat_id}
            ).execute()
            supabase.from_("user_chat").insert(
                {"userid": frnd_uid, "chatid": chat_id}
            ).execute()
    ...
    def get_all(self, user_id: int) -> Iterator[Chat]:
        ...
        with self.db.transaction() as supabase:
            supabase = cast(Client, supabase)
            res = (
                supabase.from_("get_all_contacts")
                .select("user_chatid,frnd_userid, frnd_username, chatid, blocked")
                .eq("userid", user_id)
                .execute()
            )
            chats = res.data
    ...
    def get_not_in_chat(self, user_id: int) -> tuple[Any]:
        ...
        with self.db.transaction() as supabase:
            ...
            return tuple((u["userid"], u["username"]) for u in users_resp.data)
    ...
    def update(self, user_id: int, chat_id: int, blocked: int):
        ...
        with self.db.transaction() as supabase:
            supabase = cast(Client, supabase)
            supabase.from_("user_chat").update({"blocked": blocked}).eq(
                "userid", user_id
