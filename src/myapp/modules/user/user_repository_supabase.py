...
class UserRepositorySupaBase(IUserRepo):
    ...
    def get(self, uname: str, upass: str) -> User | None:
        with self.db.transaction() as supabase:
            supabase = cast(Client, supabase)
            res = supabase.from_("users").select("userid", "username", "lastseen").eq(
                "username", uname).eq(
                    "password", self.sha512(upass)).execute()
            if res.data:
               return User(*tuple(res.data[0].values())) 
            else:
                return None
    ...
    def update(self, uid: int, new_name: str):
        with self.db.transaction() as supabase:
            supabase = cast(Client, supabase)
            supabase.from_("users").update({"username": new_name}).eq("userid", uid).execute()
