...
class MessageRepositorySupaBase(IMessageRepo):
    ...
    def add_msg(
        ...
        with self.db.transaction() as supabase :
            ...
            supabase.from_("message").insert({"message" :msg, 
                                              "userid": user_id, 
                                              "chatid": chat_id, 
                                              "groupid": grp_id, 
                                              "type": msg_type.value}
    ...
    def update_msg(
        ...
        with self.db.transaction() as supabase :
            supabase = cast(Client, supabase)
            supabase.from_("message").update({'message': updated_msg}).eq("id", msg_id).execute()
    ...
    def get_all_msg(
        ...
        with self.db.transaction() as supabase :
            supabase = cast(Client, supabase)
            query = (supabase.table("message").select("""id, message, chatid, groupid, sentat, deliveredat, seenat,
            ...
            return (Message(id = msg["id"] , 
                            ...
                            seenAt=msg['seenat']) for msg in msgs)
